...
allVedios.map(mainData => {
    ...
    // convert hours with minute form seconds part
   ...
   <p class="absolute bg-gray-800 font-light text-white rounded px-3 bottom-3 right-3">${others.posted_date ? `${hours > 12 ? 'long time ago':  `${hours}hrs ${minutes}min ago`}` : ''}</p>
   </div>
    ...
    <div class="p-3">
     ...
     <img class="w-12 h-12 rounded-full" src="${authors[0].profile_picture}" alt="" />
      <h2 class="card-title text-xl font-semibold">${title}</h2>
       ...
       <p class="ml-16 mt-2">${others.views}  views</p>
       </div>
